...
class Notedata {
  ...
  static Database? _database; // if there is any database then it will return it otherwise will create it;
  Notedata._init();
...
// This getter method is used to retrieve the SQLite database instance. If _database is not null, it returns the existing database. Otherwise, it calls _initializeDB to create and open a new database and stores it in _database.
  Future<Database?> get database async{
    ...
    _database =await _initializeDB('Notes.db'); //Notes.db is name of database for this app
    return _database;
  }
...
// It returns a hardcoded string "Succesffull" (typo: "Successful").
    Future<String> readNotes() async {
      ...
      final query_result=await db!.query("Notes"); //Notes is the name of the database 
      print(query_result);
      return "Succesffull";
    }
